...
- AK/SK
- PROJECT
- mariadb_instance_name 订购的mariadb实例的名称
- mariadb_instance_ip  订购的mariadb实例的连接地址IP
...
- 验证
  ...
  在``订单处理``页面刷新5次，将会触发熔断，在熔断时间窗内，页面再次刷新调用失败。在微服务治理页面，可以看到web处于熔断状态
